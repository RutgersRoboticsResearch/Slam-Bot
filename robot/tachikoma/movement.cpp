...
void Tachikoma::update_drive(vec v, double w) {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    this->action_motion(SHIN_IND[i])  = (shin > 0.5)  ? 1.0 : ((shin < -0.5)  ? -1.0 : 0.0);
  }
  this->action_state_finished = true;
}
  ...
  switch (this->action_step_state) {
    ...
    case 1:
      if (!within(curr_pos, this->step_f_mid)) {
        ...
        this->action_motion(SHIN_IND[legid])  = delta(ENC_SHIN);
...
vec linear_step_fcn(const vec &start, const vec &stop, double t) {
  vec path = stop - start;
  return start + path * t;
}
